...
\begin{tip}
\py{np.linspace(start, stop, num)} creates exactly \py{num} evenly spaced points including both endpoints. \py{np.arange(start, stop, step_size)} creates as many values differenting by \py{step_size} as it can fit between \py{start} and \py{stop}, starting at \py{start} and ending \textit{before} \py{stop}. Which is better really depends on usecase and needs.
\end{tip}
...
b[0, 0]      # 1 (row 0, col 0); This is easier to write than if we were using a list of lists, which would require separate square brackets around each number
b[1, 2]      # 6 (row 1, col 2)
...
\begin{note}
Writing each list (i.e. line) of your 2D array on a new line of the editor makes the code more readable, but it is not necessary: NumPy will do it automatically for you. This would not be the case with a normal list.
\end{note}
...
Vectorized functions are orders of magnitude faster for large volumes of high dimensional data than functions applied in loops would be. Scripts that can take hours using loops might take seconds with vectorized functions. NumPy provides vectorized versions of many common math functions, examples include: 
...
# Stack vertically (column by column)
np.vstack([a, b])
# array([[1, 2, 3],
#        [4, 5, 6]])
...
One of NumPy's most powerful (and often overlooked) tools is \py{np.einsum}. This function uses Einstein summation notation to represent many common array operations using a short string of letters. It is an advanced skill to develop, and is mostly added here to let you know it exists.
