...
A computer is, at its heart, a \textbf{calculator}---a machine that takes in data, follows instructions, and produces results. That's it. Every app, game, and website you've ever used is just a very elaborate sequence of instructions applied to data.

\textbf{Python} is a way to write those instructions in something close to plain English. When you type Python code, you are telling the calculator exactly \textit{what} data to work with and \textit{how} to work with it.

This means the very first thing you need to understand is: \textbf{what kinds of data can Python work with, and how does it organise that data?} Python has several built-in types for holding individual values (numbers, text, true/false), and several \textbf{data structures} for holding collections of values (lists, dictionaries, sets). We'll start with the primitive types in this chapter, then tackle containers in Chapter~\ref{chap:containers}. Trust me: you'll want to understand both, because almost every interesting problem involves \textit{collections} of values---a list of temperatures, a mapping from postcodes to suburbs, a set of unique identifiers.
...
\begin{warning}
Avoid using "magic numbers." In the examples above, if we had written \py{conc = 5.85 / 58.44 / 0.5}, it would be use of "Magic Numbers." Unexplained constants that appear out of nowhere. 

A number without a name is dangerous. Is \py{0.5} the volume? The concentration? A correction factor? By assigning \py{volume_L = 0.5}, you give that number semantic meaning.


You can learn from this warning, or from experience. How you learn this lesson is up to you, but you will learn it.
\end{warning}
...
0.30000000000000004
\end{lstlisting}
This isn't a Python bug---it's how computers work. This matters because checking for exact equality will return \py{False} when you expect \py{True} if you don't account for this.
\end{warning}
...
\begin{warning}
If you skipped ahead during the introductory chapter, now is a good time to revisit it, starting with section~\ref{sec:hygiene}. At minimum you need to read section~\ref{sec:editor}.
\end{warning}
